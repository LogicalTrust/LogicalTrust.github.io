...
Our another take was to try crash alpine via mailbox, so we crafted a super simple script with a hope that we'll find something interesting in the mailbox parsing. So we downloaded tons of spam and tried to mutate it with radamsa.
